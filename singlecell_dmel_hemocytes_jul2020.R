...
pdf("bulkgeneexp_mod_4.pdf",width = 12, height = 4)
...
mtext("1: Induced expression in populations evolved without parasitism (log2 fold change)",
      side=1,outer=T,line=-2.5,cex=0.9, at = 0.68)
...
clusterplot <- ggplot(data=integrate.combined_cellcount, aes(x=Treatment, y=Prop, fill = Parasitism, color = Infection, size= Infection)) +
  ...
  theme(legend.position = c(0.84, 1.06),legend.justification = c(0.84, 1.06)) + 
  guides(color = guide_legend(override.aes = list(fill = "white")), size = F) +
  theme(legend.margin = margin(0.4,0,0,0, unit="cm"))


##Figure 4
clusprop_lineage_plot <- plot_grid(clusterplot, lineage_plot, labels = c('A', ''), label_size = 14, ncol = 1, rel_heights = c(1.7, 1))
pdf("clusprop_lineage_go_plot.pdf", height = 15, width = 11)
plot_grid(clusprop_lineage_plot, goleftright, labels = c('', 'E'), label_size = 14, ncol = 1, rel_heights = c(1.3, 1))
dev.off()
